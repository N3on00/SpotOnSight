...
from .ids import as_text
from .mappers import to_support_ticket_public


class SocialSupportActions:
    ...
    def create_support_ticket(self, req, current_user):
    ...
    def list_all_support_tickets(self):
        rows = self.repos.support_tickets.find_all_sorted(sort_field="created_at", sort_direction=-1)
        return [to_support_ticket_public(doc) for doc in rows]

    def update_ticket_status(self, ticket_id: str, ticket_status: str):
        ...
        self.repos.support_tickets.update_fields(
            {"_id": ObjectId(ticket_id)},
            {"status": ticket_status, "updated_at": datetime.now(UTC)},
        )
    ...
    def delete_ticket(self, ticket_id: str) -> dict[str, bool]:
        if not ObjectId.is_valid(ticket_id):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid ticket ID")
        result = self.repos.support_tickets.delete_one_by_query({"_id": ObjectId(ticket_id)})
