...
from core.social import as_text, serialize_id, to_meetup_public
...
class MeetupsService(SocialServiceBase):
    def sync_meetup_invites(self, meetup_id: str, host_user_id: str, invite_user_ids: list[str]) -> None:
        ...
        self.repos.meetup_invites.collection.delete_many(
            {
                "meetup_id": meetup_id,
                "user_id": {"$nin": list(keep_ids)} if keep_ids else {"$exists": True},
            }
        )
        for invitee_id in invite_ids:
            self.repos.meetup_invites.collection.update_one(
    ...
    def list_meetup_invites(self, current_user: dict[str, Any]) -> list[MeetupInviteRef]:
        ...
        return [
            MeetupInviteRef(
                ...
                status=normalize_invite_status(row.get("status")),
